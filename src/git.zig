...
fn isLeakedEmail(email: []const u8) bool {
    const at = std.mem.lastIndexOfScalar(u8, email, '@') orelse return false;
    const domain = email[at + 1 ..];

    return std.mem.eql(u8, domain, "gmail.com") or
        std.mem.eql(u8, domain, "outlook.com") or
        std.mem.endsWith(u8, domain, ".edu");
}
...
pub fn getLinesChanged(
    ...
    var count: usize = 0;
    for (emails) |email| {
        if (!isLeakedEmail(email))
            count += 1;
    }

    const email_args = try allocator.alloc([]const u8, count * 2);
